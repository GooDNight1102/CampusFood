  ...
  if (mount){
    ...
    mount.innerHTML = `
      ...
      <div class="admin-sidebar-foot">
        <div class="admin-user-info" style="padding:10px 12px; color:rgba(255,255,255,.6); font-size:12.5px;">
        ...
        <a href="#" class="admin-back" id="admin-logout">${CF_ICONS.logout}<span>Đăng xuất</span></a>
        <a href="../index.html" class="admin-back">${CF_ICONS.home}<span>Về trang khách</span></a>
      </div>`;
...
/* Đọc file ảnh người dùng chọn, tự thu nhỏ (tối đa 900px cạnh dài) và nén
   thành JPEG chất lượng ~82% trước khi lưu — giữ localStorage nhẹ dù lưu
   nhiều ảnh, đồng thời không cần server để nhận file upload. */
function cfResizeImageToDataURL(file, maxDim = 900, quality = 0.82){
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onerror = () => reject(new Error('Không đọc được file ảnh này.'));
    reader.onload = () => {
      const img = new Image();
      ...
      img.onload = () => {
        ...
        const ctx = canvas.getContext('2d');
        if (!ctx){ reject(new Error('Trình duyệt không hỗ trợ xử lý ảnh.')); return; }
...
function cfUpdateProductImagePreview(src){
  const preview = document.getElementById('pf-image-preview');
  if (!preview) return;
  preview.innerHTML = src ? `<img src="${src}" alt="">` : CF_ICONS.bowl;
}
...
function cfRenderProductsAdmin(){
  ...
  const imageTextInput = document.getElementById('pf-image');
  const imageFileInput = document.getElementById('pf-image-file');
  imageTextInput?.addEventListener('input', () => cfUpdateProductImagePreview(imageTextInput.value.trim()));
  document.getElementById('pf-image-clear')?.addEventListener('click', () => {
    imageTextInput.value = '';
    cfUpdateProductImagePreview('');
  });
  imageFileInput?.addEventListener('change', async () => {
    const file = imageFileInput.files[0];
    if (!file) return;
    if (!file.type.startsWith('image/')){
      cfToast('Vui lòng chọn một file ảnh (jpg, png, webp...).', 'error');
      imageFileInput.value = '';
      return;
    }
    try {
      const dataUrl = await cfResizeImageToDataURL(file);
      imageTextInput.value = dataUrl;
      cfUpdateProductImagePreview(dataUrl);
      cfToast('Đã tải ảnh lên — nhớ bấm "Lưu món ăn" để hoàn tất.', 'success');
    } catch (err){
      cfToast(err.message || 'Không thể xử lý ảnh này.', 'error');
    }
    imageFileInput.value = '';
  });
...
function cfOpenProductModal(id){
  ...
  if (id){
    ...
    cfUpdateProductImagePreview(p.image || '');
  } else {
...
function cfShowOrderDetailModal(code){
  ...
  document.getElementById('od-items').innerHTML = order.items.map(i => `
    <div class="ticket-row"><span class="name">${i.name} <span class="qty">x${i.qty}</span>${i.optionsText ? `<br><span style="font-weight:400;font-size:11.5px;color:var(--ink-50);">${i.optionsText}</span>` : ''}</span><span>${cfFormatVND(i.price*i.qty)}</span></div>
  `).join('');
