...
const CF_ICONS = {
  ...
  phone: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M5 4h3l1.5 4.5-2 1.5a13 13 0 0 0 6.5 6.5l1.5-2L20 16v3a2 2 0 0 1-2 2A15 15 0 0 1 3 6a2 2 0 0 1 2-2Z"/></svg>',
  home: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 11 12 4l9 7"/><path d="M5 10v9a1 1 0 0 0 1 1h4v-6h4v6h4a1 1 0 0 0 1-1v-9"/></svg>',
  ...
  ticket: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 8a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2 2 2 0 0 0 0 4 2 2 0 0 1-2 2H6a2 2 0 0 1-2-2 2 2 0 0 0 0-4Z"/><path d="M10 6v.01M10 11v.01M10 16v.01"/></svg>',
  info: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 11v5.5M12 7.8v.01"/></svg>',
  emptycart: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="20" r="1"/><circle cx="18" cy="20" r="1"/><path d="M2 3h2l2.6 12.2A2 2 0 0 0 8.5 17h9.1a2 2 0 0 0 1.9-1.4L21.5 8H6"/><path d="m9 9 6 6m0-6-6 6" opacity=".5"/></svg>',
...
const CF_ICON_SIZED_SELECTOR = [
  ...
  '.add-btn','.tl-dot','.row-actions','.carousel-arrow','.flash-title',
  '.flash-countdown','.vc-icon','.hero-search','.ps-dot',
].join(',');
...
function cfProductCardHtml(p){
  ...
  const discountBadge = hasDiscount ? `<span class="badge badge-discount">-${discountPct}%</span>` : '';
  // Nếu món có ảnh thật (trường "image"), ảnh sẽ phủ lên trên icon minh hoạ.
  // Ảnh lỗi/chưa tải được thì tự gỡ đi, icon minh hoạ phía dưới sẽ lộ ra —
  // không bao giờ để lại một ô ảnh vỡ, trống trơn.
  const imgHtml = p.image
    ? `<img class="thumb-img" src="${p.image}" alt="${p.name}" loading="lazy" onerror="this.remove()">`
    : '';
  const priceHtml = hasDiscount
    ? `<span class="price-old">${cfFormatVND(p.oldPrice)}</span><span class="price">${cfFormatVND(p.price)}</span>`
...
function cfThumbGradient(cat){
  return { mon:'#F6DCC0,#EFA346', uong:'#CDE7DA,#5FA980', vat:'#FBE0AE,#E0592C', combo:'#FBEBD0,#EEA83A' }[cat] || '#F6DCC0,#EFA346';
}
...
/* ------------------- Nút "Xem giỏ hàng" nổi (mobile) ------------------- */
function cfRenderFloatingCart(){
  const mount = document.getElementById('floating-cart');
  if (!mount) return;
  const count = typeof cfCartCount === 'function' ? cfCartCount() : 0;
  if (count === 0){ mount.innerHTML = ''; return; }
  const total = typeof cfCartTotal === 'function' ? cfCartTotal() : 0;
  mount.innerHTML = `
    <a href="cart.html" class="floating-cart-btn">
      <span class="fc-badge">${count > 9 ? '9+' : count}</span>
      ${CF_ICONS.cart}
      <span class="fc-text">Xem giỏ hàng · ${cfFormatVND(total)}</span>
    </a>`;
}

/* ------------------- Dải icon danh mục tròn, cuộn ngang (kiểu ShopeeFood) ------------------- */
function cfCategoryBubblesHtml(activeCat){
  const cats = [
    { key:'all',   label:'Tất cả',      icon:'list',   grad:'#2E5A3E,#5C8D6E' },
    { key:'mon',   label:'Đồ ăn',       icon:'bowl',   grad:cfThumbGradient('mon') },
    { key:'uong',  label:'Đồ uống',     icon:'cup',    grad:cfThumbGradient('uong') },
    { key:'vat',   label:'Đồ ăn vặt',   icon:'fries',  grad:cfThumbGradient('vat') },
    { key:'combo', label:'Combo',       icon:'ticket', grad:cfThumbGradient('combo') },
  ];
  return cats.map(c => `
    <a href="menu.html${c.key === 'all' ? '' : '?cat=' + c.key}" class="category-bubble ${activeCat === c.key ? 'active' : ''}">
      <span class="bubble-icon" style="background:linear-gradient(135deg,${c.grad});">${CF_ICONS[c.icon]}</span>
      <span>${c.label}</span>
    </a>`).join('');
}

/* ------------------- Khung xương chờ tải (skeleton loading) ------------------- */
function cfSkeletonCardsHtml(count){
  count = count || 8;
  return Array.from({ length: count }).map(() => `
    <div class="product-card skeleton-card">
      ...
      <div class="product-body">
        ...
        <div class="skeleton-line" style="width:82%;height:17px;"></div>
        <div class="skeleton-line" style="width:60%;"></div>
        <div class="skeleton-line" style="width:45%;margin-top:10px;"></div>
      </div>
    </div>`).join('');
}

/* ------------------- Đếm ngược Flash Sale ------------------- */
function cfStartCountdown(elId, endDate){
